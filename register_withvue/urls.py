...
urlpatterns = [
    ...
    path(
        "coins/balance/<int:student_id>/", views.get_coin_balance
    ),  # GET  — bitta student balansi
    path(
        "coins/transactions/<int:student_id>/", views.get_coin_transactions
    ),  # GET  — tarix
    path(
        "coins/set/<int:student_id>/", views.set_coin_balance
    ),  # PATCH — balansni to'g'ridan belgilash (faqat manager)
    path(
        "coins/delete/<int:txn_id>/", views.delete_coin_transaction
    ),  # DELETE — tranzaksiyani bekor qilish
    # ─────────────────────────────────────
    # TEACHERS
    # ─────────────────────────────────────
